...
struct TopSellingView: View {
    ...
    var body: some View {
        VStack(alignment:.leading,spacing:10){
            Text("Top Selling").font(.system(size: 30)).bold()
            ScrollView(.horizontal,showsIndicators: false) {
                HStack(alignment:.center,spacing: 40){
                    ForEach(fruits,id: \.id){fruit in
                        NavigationLink(destination: DetailView(fruit:fruit.title),label:{
                            FruitCardView(fruit: fruit).shadow(color: .gray,radius: 5,y: 5).shadow(color: .gray.opacity(0.15), radius:5,x:10,y:10)
